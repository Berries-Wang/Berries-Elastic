...
You can use the [minimum_should_match parameter](../016.Query%20Parameter/000.minimum_should_match.md) to specify the number or percentage of should clauses returned documents must match.(您可以使用 minimum_should_match 参数指定返回的文档必须匹配的 should 子句的数量或百分比。)
